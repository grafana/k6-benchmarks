...
export default function() {
    group("Front page", function() {
        let res = http.get(http.url`http://nginx-sut.k6.io/?ts=${Math.round(randomIntBetween(1,2000))}`, { tags: { name: "http://nginx-sut.k6.io/?ts=*"}});

        let checkRes = check(res, {
            "Homepage body size is 11026 bytes": (r) => r.status === 200 && r.body.length === 619,
